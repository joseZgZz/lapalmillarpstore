  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div
          className={`relative glass-card rounded-[2.5rem] border border-white/10 px-6 py-3 flex items-center
            justify-between transition-all duration-500 ${
              isScrolled
                ? "shadow-2xl shadow-black/50 bg-[#0a0a0a]/90 backdrop-blur-xl"
                : "bg-[#0a0a0a]/40"
            }`}
        >
          {/* Logo */}
          ...
          <div className="hidden md:flex items-center gap-2">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`px-6 py-2.5 rounded-2xl text-xs font-black uppercase
            ...
            {user?.role === "admin" && (
              <Link
                to="/admin"
                className={`px-6 py-2.5 rounded-2xl text-xs font-black uppercase tracking-widest
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="pt-4 mt-4 border-t border-white/10">
                {user ? (
                  ...
                  <button
                    onClick={loginWithDiscord}
                    className="w-full py-5 bg-primary text-white rounded-3xl font-black text-sm uppercase tracking-widest"
                  >
                    CONECTAR DISCORD
                  </button>
                )}
