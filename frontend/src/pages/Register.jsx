...
import { UserPlus, Mail, Lock, User, Calendar, LogIn, Gamepad2 } from 'lucide-react';
...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Swal.fire('Error', err.response?.data?.error || 'No se pudo crear la cuenta', 'error');
        }
    };
        ...
        <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center pt-20 pb-10 px-4 relative overflow-hidden">
            ...
            >
                <div className="flex flex-col items-center mb-10 text-center">
                    ...
                    <h1 className="text-4xl font-display font-black text-white italic tracking-tighter uppercase mb-2">Crear <span className="text-primary">Cuenta</span></h1>
                    <p className="text-gray-500 font-medium">Únete a la nueva era del Roleplay premium.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="relative group">
                        <Calendar className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-primary transition-colors" size={20} />
                        <input
                            type="date"
                            className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-14 pr-5 text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all font-bold"
                ...
                <div className="mt-10 pt-8 border-t border-white/5 text-center">
                    <p className="text-gray-500 font-bold mb-4">O regístrate con servicios</p>
                    ...
                    <p className="text-gray-500 text-sm font-medium">
                        ¿Ya tienes una cuenta? <Link to="/login" className="text-primary hover:underline font-black">Inicia sesión aquí</Link>
                    </p>
                </div>
            </motion.div>
        </div>
