...
const ProductDetails = () => {
  ...
  const handlePurchase = async () => {
    ...
    const result = await Swal.fire({
      ...
      customClass: {
        confirmButton:
          "font-black uppercase tracking-widest text-xs py-4 px-8 rounded-xl",
        cancelButton:
          "font-black uppercase tracking-widest text-xs py-4 px-8 rounded-xl",
      },
    });
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          ...
          customClass: {
            confirmButton:
              "w-full font-black uppercase tracking-widest text-xs py-4 rounded-xl",
          },
        });
      } catch (err) {
        Swal.fire(
          "Error",
          err.response?.data?.error || "Fallo en la transacción",
          "error",
        );
      }
    }
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-[#0a0a0a] text-white">
        ...
        <button
          ...
          className="btn-card-secondary px-8 py-3"
        >
          VOLVER A LA TIENDA
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-7 flex flex-col gap-8">
            <motion.div
              ...
              className="glass-card rounded-[3rem] overflow-hidden border border-white/5 relative bg-black/50"
            >
              ...
              <div className="absolute top-8 left-8 z-20">
                <div className="bg-[#ff2e2e] text-white text-[10px] font-black px-5 py-2 rounded-xl shadow-2xl uppercase tracking-widest">
                  RECURSO PREMIUM
                </div>
              </div>
            </motion.div>
            ...
            <div className="glass-card rounded-[3rem] p-10 border border-white/5">
              <h2 className="text-2xl font-display font-black text-white mb-6 border-b border-white/5 pb-6 flex items-center gap-4">
                <Info className="text-primary" size={24} /> Descripción del
                activo
              </h2>
          ...
          >
            <div className="glass-card rounded-[3rem] p-10 glow-border border-[#ff2e2e]/20 relative overflow-hidden">
              ...
              <h1 className="text-4xl font-display font-black text-white mb-6 tracking-tight leading-tight uppercase">
                {product.name}
              </h1>
