...
const Admin = () => {
    ...
    const handleSaveProduct = async (e) => {
        ...
        try {
            ...
            setIsEditing(null);
            fetchData();
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: '¡Guardado!', showConfirmButton: false, timer: 2000, background: '#0a0a0a', color: '#fff' });
    ...
    const handleCreateCategory = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post(`${API_URL}/api/categories`, { name: catNameInput, icon: 'Package' }, { headers: { Authorization: `Bearer ${token}` } });
            setCatNameInput('');
            fetchData();
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Categoría Creada', background: '#0a0a0a', color: '#fff' });
        } catch (err) { }
    };
    ...
    const handleDeleteCategoryByName = async () => {
        ...
        if (!cat) return Swal.fire('No encontrada', 'Escribe el nombre exacto', 'error');
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${API_URL}/api/categories/${cat._id}`, { headers: { Authorization: `Bearer ${token}` } });
            setCatNameInput('');
            fetchData();
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Borrada', background: '#0a0a0a', color: '#fff' });
        } catch (err) { }
    };
    ...
    const handleCoinsAction = async (action) => {
        try {
            const token = localStorage.getItem('token');
            await axios.post(`${API_URL}/api/users/manage-coins`, { ...manualBalanceForm, action }, { headers: { Authorization: `Bearer ${token}` } });
            setManualBalanceForm({ username: '', amount: '', action: 'add' });
            fetchData();
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Balance Actualizado', background: '#0a0a0a', color: '#fff' });
        ...
        return (
            <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4">
                <div className="glass-card p-10 text-center max-w-md">
                    ...
                    <h1 className="text-2xl font-black text-white mb-4 uppercase">ACCESO RESTRINGIDO</h1>
                    <p className="text-gray-500 mb-8 font-bold">No tienes permisos de administrador o no has iniciado sesión.</p>
                    <button onClick={() => window.location.href = '/'} className="btn-card-primary px-8 py-3">VOLVER AL INICIO</button>
                </div>
            </div>
        );
    }
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <header className="mb-16">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <span className="text-[10px] font-black text-gray-500 uppercase tracking-[0.4em]">SISTEMA_CORE_V3.1</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    ...
                    <div className="lg:col-span-5 space-y-8">
                        ...
                        <section className="glass-card rounded-[3.5rem] p-10 border border-white/5 bg-[#111]/40">
                            ...
                            <form onSubmit={handleSaveProduct} className="space-y-4">
                                ...
                                <input type="text" placeholder="URL Imagen Principal" className="admin-input" value={prodForm.image} onChange={e => setProdForm({ ...prodForm, image: e.target.value })} required />
                                ...
                                {isEditing && <button type="button" onClick={() => { setIsEditing(null); setProdForm({ name: '', description: '', price: '', image: '', images: '', category: '' }); }} className="w-full mt-2 text-gray-500 font-bold text-[10px] uppercase">CANCELAR</button>}
                            </form>
                        </section>
                        ...
                        <section className="glass-card rounded-[3.5rem] p-10 border border-white/5 bg-secondary/[0.02]">
                            ...
                            <p className="text-[10px] text-gray-500 font-black mb-8 uppercase tracking-widest leading-loose">Escribe el nombre y usa los botones para añadir o quitar secciones de la tienda.</p>
                            <div className="space-y-4">
                                <input type="text" placeholder="VIP, Vehículos, Dinero..." className="admin-input border-secondary/20" value={catNameInput} onChange={e => setCatNameInput(e.target.value)} />
                                <div className="flex gap-4">
                                    <button onClick={handleCreateCategory} className="flex-1 py-4 bg-secondary text-black rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-secondary/10">AÑADIR</button>
                                    <button onClick={handleDeleteCategoryByName} className="flex-1 py-4 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-primary/10">ELIMINAR</button>
                                </div>
                            </div>
                            <div className="mt-8 flex flex-wrap gap-2">
                                {categories.map(c => (
                                    <span key={c._id} onClick={() => setCatNameInput(c.name)} className="px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] font-black text-gray-500 hover:text-secondary hover:border-secondary cursor-pointer transition-all uppercase">{c.name}</span>
                                ))}
                            </div>
                        </section>
                    ...
                    <div className="lg:col-span-7 space-y-10">
                        <section className="glass-card rounded-[3.5rem] p-10 border border-white/5">
                            <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.4em] mb-10">Artículos en Venta</h3>
                            <div className="space-y-4 max-h-[600px] overflow-y-auto pr-4 no-scrollbar">
                                {products.map(p => (
                                    <div key={p._id} className="flex items-center justify-between p-5 bg-white/[0.01] border border-white/5 rounded-3xl group">
                                        ...
                                        <div className="flex gap-2">
                                            <button onClick={() => { setIsEditing(p._id); setProdForm({ name: p.name, description: p.description, price: p.price, image: p.image, images: p.images?.join(', ') || '', category: p.category }); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="p-3 text-gray-500 hover:text-secondary bg-white/5 rounded-xl"><Edit3 size={18} /></button>
                                            <button onClick={async () => {
                                                const res = await Swal.fire({ title: '¿Borrar?', icon: 'warning', showCancelButton: true, background: '#0a0a0a', color: '#fff' });
                                                if (res.isConfirmed) {
                                                    const token = localStorage.getItem('token');
                                                    await axios.delete(`${API_URL}/api/products/${p._id}`, { headers: { Authorization: `Bearer ${token}` } });
                                                    fetchData();
                                                }
                                            }} className="p-3 text-gray-500 hover:text-primary bg-white/5 rounded-xl"><Trash2 size={18} /></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <section className="glass-card rounded-[3.5rem] p-10 border border-white/5">
                            <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.4em] mb-8">Gestión de Coins</h3>
                            <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                <input type="number" placeholder="Coins" className="admin-input w-28" value={manualBalanceForm.amount} onChange={e => setManualBalanceForm({ ...manualBalanceForm, amount: e.target.value })} />
                                <div className="flex gap-2">
                                    <button onClick={() => handleCoinsAction('add')} className="px-6 py-4 bg-secondary text-black rounded-2xl font-black text-[10px] uppercase">DAR</button>
                                    <button onClick={() => handleCoinsAction('remove')} className="px-6 py-4 bg-primary text-white rounded-2xl font-black text-[10px] uppercase">QUITAR</button>
                                </div>
                            </div>
                        </section>
                    </div>
